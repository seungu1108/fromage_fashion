...
class LLaMA_adapter(nn.Module):
    ...
    def __init__(self, llama_ckpt_dir, llama_tokenizer,
                 #max_seq_len= 512, #hjnam
                 max_seq_len= 128, #hjnam
                 max_batch_size=1,  
                 clip_model='ViT-L/14',
    ...
    def generate(
        self, imgs, prompts,
        max_gen_len: int = 256,  #hjnam
        #max_gen_len: int = 30, #80,   #80/100
...
def load(name, llama_dir, device="cuda" if torch.cuda.is_available() else "cpu", download_root='ckpts', max_seq_len=512,  #hjnam
#def load(name, llama_dir, device="cuda" if torch.cuda.is_available() else "cpu", download_root='ckpts', max_seq_len=128,
        phase="finetune"):
    if name in _MODELS:
    ...
    model = LLaMA_adapter(
        ...
        #max_seq_len=512, max_batch_size=1, #hjnam
